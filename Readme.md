...
* 5 test predictions, in Kaggle format ("_results_modelname.csv")
* One pdf file of the final report.
...
## Description
In this project, we implemented several machine learning models to predict the binned IMDB score of a movie given several attributes. Five machine learning models were chosen: Naive Bayes, k-NN, Decision Tree, Random Forest, and Stacking Classifier.
The implementation of these models are done by utilising functions from the sci-kit learn library.
Prior to model training, the dataset was preprocessed, going through several steps to enhance model performance. The objective of this project is to implement a model that produces the highest accuracy possible to make predictions.

## Getting Started

### Dependencies
* It's necessary to load all the required libraries in the first section of the Jupyter notebook file.
* This Jupyter notebook was run on Mac OS machine.
* This model is made to handle training and test files as provided, "train_dataset.csv" and "test_dataset.csv".

### Installing
* Use your preferred package manager to install all package requirements.

### Executing
* After unzipping file, locate "COMP30027_2024_project2.ipynb" in your repository.
* Make sure that the train and test files are in the same repository as the Jupyter notebook file.
* Each section of the code in the Jupyter notebook file should be run sequentially.

## Help
* Please contact the authors if experiencing issues.

## Authors
* Jerrilyn Looi (jklooi@student.unimelb.edu.au)
* Melissa Putri (melissadiand@student.unimelb.edu.au)
